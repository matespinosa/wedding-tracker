...
- Google Sheets is the source of truth once the Apps Script Web App URL and token are configured. The app reads on startup, keeps local edits in a retry queue while offline, and writes `Corte` / `Iglesia` changes back by `id` or `clave`.
