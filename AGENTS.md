...
- The Iglesia module uses exactly two chip-style tabs: `Ceremonia` first and `Corte ceremonial` second.
- `Ceremonia` opens by default. Its ceremony-time card is the first and most prominent item, followed by individual cards for officiant, decoration, the church-protocol checklist, ceremony-photography checklist, newlyweds' car checklist, and other ceremony details.
- `Corte ceremonial` groups individual cards by `Dama de la corte`, `Caballero de la corte`, `Testigo`, and `Pajecito`. Each person owns a separate card rather than being represented by one aggregate task.
- Tapping any Iglesia card opens a phone-scoped bottom sheet that first shows the current information without opening the keyboard. The same sheet lets the user update and save the record, including status and responsible party where relevant.
- Google Sheets is the source of truth and the connection is baked into the build: nobody types a URL or token. Reads from the Web App are public; the token is only used for writes. The app polls `Tracker_Version` through a published CSV every 12s and only downloads the full JSON when that number changes, with a forced full read every five minutes. It keeps local edits in a retry queue while offline, and writes `Corte` / `Iglesia` changes back by `id` or `clave`.
- Tasks use three primary states and must always distinguish them with text, a color dot, and visual treatment: `Pendiente` (neutral), `En progreso` (terracotta), and `Listo` (sage green). A task's state chip is actionable and advances in that order; the state is also persisted to Google Sheets.
- Guests are two separate lists, never one mixed one. `Invitados` opens on tabs `Recepción` and `Iglesia`, each showing its own head count. The church is open-door: those guests never confirm attendance and never pick a meal, so their cards hide the RSVP pill and their detail sheet hides Confirmación and Plato. Every RSVP figure in the app — the summary hero, the KPIs, the pending panel, the reception module — counts only guests whose `invitado_a` is `Recepción` or `Ambas`.
- `Invitado a` is the first control in the guest detail sheet and is also chosen when adding a guest, because moving people between the two lists is how the couple untangles a mixed list.
- Tapping any `Pendiente` row (its title area, marked with a chevron) opens a detail sheet that edits title, information/notes, due date, state, responsible party, and priority, and can delete the task. Deleting is armed by a first tap and confirmed by a second; any other edit in the sheet disarms it. Deletes reach Google Sheets as `accion: "borrar"` and queue offline like every other change.
- The bottom navigation is a floating iOS-style liquid-glass capsule: inset from the screen edges, translucent over a blurred and saturated backdrop, with a specular top highlight and a lighter glass pill behind the active item. Content scrolls behind it; where `backdrop-filter` is unsupported it falls back to opaque.
